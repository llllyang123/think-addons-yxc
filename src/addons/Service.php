...
class Service extends \think\Service
{
    ...
    //根据数据库的插件信息来加载插件
    //
    public function sql_auto_plugin($config){
        // 读取插件目录及钩子列表
        $base = Db::name('plugin')->where('status',1)->select();
        // $base = Db::name('plugin')->where('status',1)->cache('addons_list')->select();
        
        // 循环将钩子方法写入配置中
        foreach ($base as $hook) {
            if (!isset($config['hooks'][$hook['name'].'hook'])) {
                $config['hooks'][$hook['name'].'hook'] = [];
            }
            // 兼容手动配置项
            if (is_string($config['hooks'][$hook['name'].'hook'])) {
                $config['hooks'][$hook['name'].'hook'] = explode(',', $config['hooks'][$hook['name'].'hook']);
            }
            if (!in_array($hook['name'], $config['hooks'][$hook['name'].'hook'])) {
                $config['hooks'][$hook['name'].'hook'][] = $hook['name'];
            }
        }
        Config::set($config, 'addons');
    }
